...
def main(config):
    ...
    # Initialize model
    model = deeplabv3plus_resnet101(num_classes=num_classes, output_stride=8, pretrained_backbone=True)
...
def train_val(config, model, train_loader, val_loader, criterion):
    ...
    train_dice = DiceMetric(include_background=True, num_classes=num_classes, reduction="mean")
        ...
        for idx, (batch, batch_w_s) in enumerate(train_loop):
            ...
            if epoch < warmup_epochs:
                ...
                with torch.no_grad():
                    ...
                    num_classes = weak_batch.size(1)  # from model output channels
                    pseudo_labels_one_hot = F.one_hot(pseudo_labels, num_classes=num_classes).permute(0, 3, 1, 2).float()
...
if __name__ == '__main__':
    ...
    # Ưu tiên GPU; không có CUDA thì chạy CPU
    ...
    if device.type == 'cuda':
        ...
        print("(Chạy trên CPU. Muốn dùng GPU: pip uninstall torch && pip install torch --index-url https://download.pytorch.org/whl/cu118)")
